    ...
    methods: {
      ...
      async setSource(e, index) {
        ...
        console.log("this.fabricObject===",this.fabricObject);
          await this.fabricObject.initPdf(source ? source : '', index);
          console.log("this.fabricObject2===",this.fabricObject);
          this.onChange('url');
      },
      setAutoSize(state) {
        if (state) {
  ...
  <div class="pdf-controls">
    ...
    <div class="inputs-row">
        <div class="input-block">
          ...
          <div>
            <FileInput v-for="(url, index) in fabricObject.otherUrls" :index="index" :icon="'images/ic-image.svg'" :extensions="['pdf']" :value="url" @change="(e) => setSource(e, index)"  />
          </div>
        </div>
      </div>
